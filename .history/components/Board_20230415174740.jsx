...
export default function Board() {
  ...
  function moveLinkedList(originalLinkedlist) {
    ...
    if (newHead.value === foodCell) {
      let growthCell = createNewNodeInDirection(board, newHead);
      linkedList.next = growthCell;
      while (true) {
        let newFoodCell = randomIntInIntervel(1, board[14][14]);
        if (snakeCells.has(newFoodCell)) {
          setFoodCell(randomIntInIntervel(1, board[14][14]));
          break;
        }
      }
      return { head: growthCell, tail: linkedList.tail.next };
    }
