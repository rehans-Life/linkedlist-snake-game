...
export default function Board() {
  ...
  function createNewNodeInDirection(board, currHead) {
    let newHead = false;
    if (direction === "LEFT" && currHead.col - 1 >= 0)
      newHead = new Node(
        board[currHead.row][currHead.col - 1],
        currHead.row,
        currHead.col - 1
      );
    else if (direction === "RIGHT" && currHead.col + 1 < 15)
      newHead = new Node(
        board[currHead.row][currHead.col + 1],
        currHead.row,
        currHead.col + 1
      );
    else if (direction === "DOWN" && currHead.row + 1 < 15)
      newHead = new Node(
        board[currHead.row + 1][currHead.col],
        currHead.row + 1,
        currHead.col
      );
    else if (direction === "UP" && currHead.row - 1 >= 0)
      newHead = new Node(
        board[currHead.row - 1][currHead.col],
        currHead.row - 1,
        currHead.col
  ...
  function gameover() {
    ...
    setLinkedList(new SinglyLinkedList({ value: 80, row: 5, col: 4 }));
    setSnakeCells(new Set([80]));
    setFoodCell(85);
  }
