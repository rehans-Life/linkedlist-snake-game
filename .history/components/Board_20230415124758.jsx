...
export default function Board() {
  ...
  useEffect(() => {
    setInterval(() => {
      console.log(new Array(snakeCells));
    }, 1000);
  }, []);

  function moveSnake() {
    let movedSnake = new Array(snakeCells).map((value) => value + 1);
    let movedLinkedList = linkedlist.slice();
    let currHead = movedLinkedList.head;
    let newHead = new Node(currHead.value + 1);
    currHead.next = newHead;
    movedLinkedList.head = newHead;
    movedLinkedList.tail = linkedlist.tail.next;
    setSnakeCells(movedSnake);
    setLinkedList(movedLinkedList);
  }
